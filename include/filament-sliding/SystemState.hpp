...
private:
  ...
  // stretch at a time (at exactly 1.5, there could be 4). The number is defined
  // smaller than 1.5 to be sure that there are never 4 states possible, in
  // which the state could become locked. 15 decimals seems to be the precision
  ...
  ExternalForceType
      m_externalForceType; // not const, has to be found in the constructor body
  const double m_externalForceValue;

  double externalForceFlatOptimalPath() const;

  const double m_pi = std::acos(
      -1); // used for the calculation of sinus, for a sinusoidal external force
public:
  SystemState(const double lengthMobileMicrotubule,
              const double lengthFixedMicrotubule, const double latticeSpacing,
  ...
  Crosslinker &
  connectFreeCrosslinker(const Crosslinker::Type type,
  ...
  void connectPartiallyConnectedCrosslinker(
      Crosslinker &crosslinker, const SiteLocation locationOppositeMicrotubule);
  ...
  std::pair<int32_t, int32_t> getNPartialCrosslinkersBoundWithHeadAndTailOfType(
      const Crosslinker::Type type) const;

  int32_t getNFullCrosslinkers() const;
  int32_t getNFullRightPullingCrosslinkers() const;

  // The following functions return the position of the overlap as measured from
  // the origin at the beginning of the fixed microtubule
  ...
  // whichFreeSite gives the location in the storage of the Microtubule objects,
  // does not say anything about which physical site that is.
  int32_t getFreeSitePosition(const MicrotubuleType microtubuleType,
...
#ifdef MYDEBUG
  void
  TESTunbindAFullCrosslinker(const int32_t which,
                             const Crosslinker::Terminus terminusToDisconnect,
                             const Crosslinker::Type typeToDisconnect);
#endif // MYDEBUG

  const std::vector<PossibleFullConnection> &
  getPossibleConnections(const Crosslinker::Type type) const;

  const std::vector<PossiblePartialHop> &
  getPossiblePartialHops(const Crosslinker::Type type) const;

  const std::vector<PossibleFullHop> &
  getPossibleFullHops(const Crosslinker::Type type) const;

  const std::vector<FullConnection> &
  getFullConnections(const Crosslinker::Type type) const;

  const std::vector<Crosslinker *> &
  getPartialLinkers(const Crosslinker::Type type) const;

  const std::vector<Crosslinker *> &
  getFullLinkers(const Crosslinker::Type type) const;

  const std::vector<Crosslinker *> &
  getPartialLinkersBoundWithHead(const Crosslinker::Type type) const;

  const std::vector<Crosslinker *> &
  getPartialLinkersBoundWithTail(const Crosslinker::Type type) const;
