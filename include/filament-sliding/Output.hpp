...
/* Output contains all output files, and contains functions to output
 * information to those. Further, it can do some data analysis. The results of
 * this should then also be stored in a file.
 */
...
private:
  ...
  double m_lastCrossingTime; // Necessary, since we want statistics on the time
                             // interval between the previous and next crossing
  ...
  double
  calculateReactionCoordinate(const double remainder,
...
public:
  Output(const std::string &runName, const bool writePositionalDistribution,
         ...
         const double maxPeriodPositionTracking, const double latticeSpacing,
  ...
  void writeMicrotubulePosition(const double time,
                                const SystemState &systemState);

  void addPositionAndConfiguration(const double remainder,
                                   const int32_t nRightPullingCrosslinkers);

  void writeBarrierCrossingTime(const double time, const int32_t direction);

  void addPointTransitionPath(const double time, const double mobilePosition,
                              const int32_t nRightPullingCrosslinkers);

  void addPositionAndConfigurationTransitionPath(
      const double remainder, const int32_t nRightPullingCrosslinkers);

  void addTimeStepToPeakAnalysis(const double remainder,
                                 const int32_t nRightPullingCrosslinkers);
