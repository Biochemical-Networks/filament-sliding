...
class Output {
  private:
    ...
    double m_lastCrossingTime; // Necessary, since we want statistics on the
                               // time interval between the previous and next
                               // crossing
    ...
    double calculateReactionCoordinate(
            const double remainder,
            const int32_t nRightPullingCrosslinkers) const;

    bool reactionCoordinateIsAtPeakRegion(
            const double reactionCoordinate) const;
    bool reactionCoordinateLeftPeakRegion(
            const double reactionCoordinate) const;
  ...
  public:
    ...
    void addTimeStepToPeakAnalysis(
            const double remainder,
            const int32_t nRightPullingCrosslinkers);
