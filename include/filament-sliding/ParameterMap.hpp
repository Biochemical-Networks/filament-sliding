...
private:
  ...
  template <typename T>
  void defineParameter(const std::string name, T value,
                       std::string unit = "unitless",
                       std::string possibleValues = "all") {
    m_parameterOrder.push_back(
        name); // Add the name to this FIFO container, such that it can be
               // printed in the right order
    m_parameterMap.insert(
        {name,
         GenericValue{value, unit}}); // The map uses the name as a key and the
                                      // GenericValue as the value.
...
public:
  ...
  void copyParameter(const std::string &name, T &variable) {
    GenericValue variableType{
        variable,
        "unknown_unit"}; // Create a GenericValue with the type of variable
    // Change a std::out_of_range to an InputException, such that the exceptions
    // can be handled uniformly.
    try {
      variableType = m_parameterMap.at(
          name); // Call the assignment operator. This operation will throw if
                 // the types don't match
    } catch (const std::out_of_range &error) // Throw if m_parameterMap does not
                                             // contain the key 'name'
    {
      throw InputException{"Out of range error: the parameterMap does not "
  ...
  void overrideParameter(const std::string name, T value) {
                                             ...
                                             // contain the key 'name'
    {
      throw InputException{
          "Out of range error: the parameterMap does not contain a variable "
          "with the name " +
          name + ", which overrideParameter() is trying to access.\n"};
    }
  }
