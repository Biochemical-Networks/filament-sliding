...
private:
  ...
  // CircleShape cannot be part of a vertex array, it is one itself. Draw needs
  // to be called on each separate circle
  std::vector<sf::CircleShape> m_circles;
  sf::VertexArray m_lines;
  virtual void draw(sf::RenderTarget &target, sf::RenderStates states) const;

public:
  MicrotubuleGraphic(const int32_t nSites, const float circleRadius,
                     const float lineLength, const float lineThickness,
