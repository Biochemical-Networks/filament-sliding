...
/* The Reaction that unbinds a partially connected linker such that it becomes
 * free. This class needs to decide which partial linker disconnects, and only
 * reports the total unbinding rate to the public.
 */
...
private:
  ...
  // when it involves unbinding the head (tail).
  const double
      m_headUnbindingFactor; // Equals 2/(1+exp(headBindingBiasEnergy)) =
                             // 2*prob_tail_binds. m_tailUnbindingFactor =
                             // 2-m_headUnbindingFactor
  const double m_tailUnbindingFactor;

  Crosslinker &whichToDisconnect(SystemState &systemState,
                                 RandomGenerator &generator) const;

public:
  UnbindPartialCrosslinker(const double rateOneTerminusDisconnects,
