...
private:
  ...
  const int32_t
      m_positionProbePeriod; // Holds the number of time steps that are between
                             // two probes of the system, meaning that data will
                             // be written every so many time steps
  const double m_diffusionConstantMicrotubule;
  const double m_springConstant;
  const double m_latticeSpacing;

  const double m_deviationMicrotubule; // sqrt(2 D t), with t the time step size
                                       // and D the diffusion constant of the
                                       // microtubule. Stored to prevent this
                                       // calculation every time step
  ...
  // with just the objects. std::unique_ptr deletes the thing it is pointing to
  // when going out of scope, meaning that we don't have to worry about memory
  // leaks
  ...
  void propagateBlock(SystemState &systemState, RandomGenerator &generator,
                      Output &output, const bool writeOutput,
                      const int32_t nTimeSteps);

  bool inBasinOfAttraction(const double mobilePosition,
...
public:
  Propagator(
      ...
      const int32_t probePeriod, const double diffusionConstantMicrotubule,
      const double springConstant, const double latticeSpacing,
      const double ratePassivePartialHop, const double ratePassiveFullHop,
      const double baseRateActivePartialHop, const double baseRateActiveFullHop,
      const double activeHopToPlusBiasEnergy, const double neighbourBiasEnergy,
  ...
  void run(SystemState &systemState, RandomGenerator &generator,
           Output &output);
  void equilibrate(SystemState &systemState, RandomGenerator &generator,
                   Output &output);
  void propagateGraphicsInterval(SystemState &systemState,
