...
#include "filament-sliding/Graphics.hpp"
#include "filament-sliding/Initialiser.hpp"
...
int main(int argc, char *argv[]) {
  ...
  Input input(
      invokerInputHandler); // Read the input file, ask to create a default one
                            // when anything is wrong with it (e.g. nonexistent)
  ...
  std::string runName =
      input.getRunName(); // copyParameter("runName", runName) would give the
                          // runName as it is in the input file, while this is
                          // the unique version

  // Write this to the standard output, not to the Log file: it can be important
  // to identify which run corresponds to which process. On the cluster, this
  // get written to a file that also contains the job-ID, linking the ID to the
  // runName
  ...
  if (lengthMobileMicrotubule <= 0.0) {
    throw GeneralException(
        "The parameter lengthMobileMicrotubule contains a wrong value.");
  }
  ...
  if (lengthFixedMicrotubule <= 0.0) {
    throw GeneralException(
        "The parameter lengthFixedMicrotubule contains a wrong value.");
  }
  ...
  if (latticeSpacing <= 0.0) {
    throw GeneralException(
        "The parameter latticeSpacing contains a wrong value.");
  }
  ...
  if (maximumStretchPerLatticeSpacing <= 0.0) {
    throw GeneralException("The parameter maximumStretchPerLatticeSpacing "
  ...
  if (nActiveCrosslinkers < 0) {
    throw GeneralException(
        "The parameter nActiveCrosslinkers contains a wrong value.");
  }
  ...
  if (nDualCrosslinkers < 0) {
    throw GeneralException(
        "The parameter nDualCrosslinkers contains a wrong value.");
  }
  ...
  if (nPassiveCrosslinkers < 0) {
    throw GeneralException(
        "The parameter nPassiveCrosslinkers contains a wrong value.");
  }
  ...
  if (springConstant < 0.0) {
    throw GeneralException(
        "The parameter springConstant contains a wrong value.");
  }

  std::string addExternalForceString;
  input.copyParameter("addExternalForce", addExternalForceString);
  const bool addExternalForce =
      (addExternalForceString ==
       "TRUE"); // Whenever it is not TRUE, assume it is false
  ...
  SystemState systemState(
      lengthMobileMicrotubule, lengthFixedMicrotubule, latticeSpacing,
      maximumStretchPerLatticeSpacing, nActiveCrosslinkers, nDualCrosslinkers,
      nPassiveCrosslinkers, springConstant, addExternalForce,
      externalForceTypeString, externalForceValue);

  //-----------------------------------------------------------------------------------------------------
  // Create the output class. Needs to be done before the propagator, since this
  // needs samplePositionalDistribution as well
  ...
  input.copyParameter("samplePositionalDistribution",
                      samplePositionalDistributionString);
  // Whenever it is not TRUE, assume it is false.
  // Also overwrite sampling the positional distribution even when it is turned
  // off when recordTransitionPaths is true. This is a choice made in the output
  // class, and setting samplePositionalDistribution to true makes sure those
  // assumptions are satisfied
  const bool samplePositionalDistribution =
      (samplePositionalDistributionString == "TRUE") || recordTransitionPaths;
  ...
  if (samplePositionalDistribution && positionalHistogramBinSize <= 0.0) {
    throw GeneralException(
        "The parameter positionalHistogramBinSize contains a wrong value.");
  }

  double positionalHistogramLowestValue;
  input.copyParameter("positionalHistogramLowestValue",
                      positionalHistogramLowestValue);
  if (positionalHistogramLowestValue < 0.0) {
    throw GeneralException(
        "The parameter positionalHistogramLowestValue contains a wrong value.");
  }

  double positionalHistogramHighestValue;
  input.copyParameter("positionalHistogramHighestValue",
                      positionalHistogramHighestValue);
  if (positionalHistogramHighestValue <= positionalHistogramLowestValue) {
    throw GeneralException("The parameter positionalHistogramHighestValue "
                           "contains a wrong value.");
  }
  ...
  if (bindingDynamics) {
    maxNFullCrosslinkers = std::min(systemState.getNFreeSitesFixed(),
                                    systemState.getNFreeSitesMobile());
  } else {
...
#ifdef MYDEBUG
  std::cout << "The maximum number of full linkers is given by "
            << maxNFullCrosslinkers << ".\n";
#endif // MYDEBUG
  ...
  if (transitionPathProbePeriod <= 0) {
    throw GeneralException(
        "The parameter transitionPathProbePeriod contains a wrong value.");
  }
  ...
  if (maxNumberTransitionPaths < 0) {
    throw GeneralException(
        "The parameter maxNumberTransitionPaths contains a wrong value.");
  }
  ...
  if (maxPeriodPositionTracking < 0.0) {
    throw GeneralException(
        "The parameter maxPeriodPositionTracking contains a wrong value.");
  }

  std::string estimateTimeEvolutionAtPeakString;
  input.copyParameter("estimateTimeEvolutionAtPeak",
                      estimateTimeEvolutionAtPeakString);
  const bool estimateTimeEvolutionAtPeak =
      (estimateTimeEvolutionAtPeakString == "TRUE");

  int32_t timeStepsPerDistributionEstimate;
  input.copyParameter("timeStepsPerDistributionEstimate",
                      timeStepsPerDistributionEstimate);
  if (estimateTimeEvolutionAtPeak && timeStepsPerDistributionEstimate <= 0) {
    throw GeneralException("The parameter timeStepsPerDistributionEstimate "
                           "contains a wrong value.");
  }
  ...
  if (estimateTimeEvolutionAtPeak && nEstimatesDistribution <= 0) {
    throw GeneralException(
        "The parameter nEstimatesDistribution contains a wrong value.");
  }

  double dynamicsEstimationInitialRegionWidth;
  input.copyParameter("dynamicsEstimationInitialRegionWidth",
                      dynamicsEstimationInitialRegionWidth);
  if (estimateTimeEvolutionAtPeak &&
      (dynamicsEstimationInitialRegionWidth <= 0.0 ||
       dynamicsEstimationInitialRegionWidth > 1.0)) {
    throw GeneralException("The parameter dynamicsEstimationInitialRegionWidth "
                           "contains a wrong value.");
  }

  double dynamicsEstimationFinalRegionWidth;
  input.copyParameter("dynamicsEstimationFinalRegionWidth",
                      dynamicsEstimationFinalRegionWidth);
  if (estimateTimeEvolutionAtPeak &&
      (dynamicsEstimationFinalRegionWidth <= 0.0 ||
       dynamicsEstimationFinalRegionWidth > 1.0)) {
    throw GeneralException("The parameter dynamicsEstimationFinalRegionWidth "
                           "contains a wrong value.");
  }

  Output output(runName, samplePositionalDistribution, recordTransitionPaths,
                transitionPathProbePeriod, maxNumberTransitionPaths,
                positionalHistogramBinSize, positionalHistogramLowestValue,
                positionalHistogramHighestValue, maxNFullCrosslinkers,
                maxPeriodPositionTracking, latticeSpacing,
                estimateTimeEvolutionAtPeak, timeStepsPerDistributionEstimate,
                nEstimatesDistribution, dynamicsEstimationInitialRegionWidth,
                dynamicsEstimationFinalRegionWidth);
  ...
  input.copyParameter("fractionOverlapSitesConnected",
                      fractionOverlapSitesConnected);
  if (fractionOverlapSitesConnected < 0.0 ||
      fractionOverlapSitesConnected > 1.0) {
    throw GeneralException(
        "The parameter fractionOverlapSitesConnected contains a wrong value.");
  }

  std::string initialCrosslinkerDistributionString;
  input.copyParameter("initialCrosslinkerDistribution",
                      initialCrosslinkerDistributionString);
  ...
  if (numberEquilibrationBlocks < 0) {
    throw GeneralException(
        "The parameter numberEquilibrationBlocks contains a wrong value.");
  }
  ...
  if (numberRunBlocks < 0) {
    throw GeneralException(
        "The parameter numberRunBlocks contains a wrong value.");
  }
  ...
  if (calcTimeStep <= 0.0) {
    throw GeneralException(
        "The parameter calcTimeStep contains a wrong value.");
  }
  ...
  if (positionProbePeriod <= 0) {
    throw GeneralException(
        "The parameter positionProbePeriod contains a wrong value.");
  }

  double diffusionConstantMicrotubule;
  input.copyParameter("diffusionConstantMicrotubule",
                      diffusionConstantMicrotubule);
  if (diffusionConstantMicrotubule < 0.0) {
    throw GeneralException(
        "The parameter diffusionConstantMicrotubule contains a wrong value.");
  }
  ...
  if (ratePassivePartialHop < 0.0) {
    throw GeneralException(
        "The parameter ratePassivePartialHop contains a wrong value.");
  }
  ...
  if (ratePassiveFullHop < 0.0) {
    throw GeneralException(
        "The parameter ratePassiveFullHop contains a wrong value.");
  }
  ...
  if (baseRateActivePartialHop < 0.0) {
    throw GeneralException(
        "The parameter baseRateActivePartialHop contains a wrong value.");
  }
  ...
  if (baseRateActiveFullHop < 0.0) {
    throw GeneralException(
        "The parameter baseRateActiveFullHop contains a wrong value.");
  }
  ...
  input.copyParameter("baseRateZeroToOneExtremitiesConnected",
                      baseRateZeroToOneExtremitiesConnected);
  if (baseRateZeroToOneExtremitiesConnected < 0.0) {
    throw GeneralException(
        "The parameter baseRateZeroToOneExtremitiesConnected contains a wrong "
        "value.");
  }

  double baseRateOneToZeroExtremitiesConnected;
  input.copyParameter("baseRateOneToZeroExtremitiesConnected",
                      baseRateOneToZeroExtremitiesConnected);
  if (baseRateOneToZeroExtremitiesConnected < 0.0) {
    throw GeneralException(
        "The parameter baseRateOneToZeroExtremitiesConnected contains a wrong "
        "value.");
  }

  double baseRateOneToTwoExtremitiesConnected;
  input.copyParameter("baseRateOneToTwoExtremitiesConnected",
                      baseRateOneToTwoExtremitiesConnected);
  if (baseRateOneToTwoExtremitiesConnected < 0.0) {
    throw GeneralException("The parameter baseRateOneToTwoExtremitiesConnected "
                           "contains a wrong value.");
  }

  double baseRateTwoToOneExtremitiesConnected;
  input.copyParameter("baseRateTwoToOneExtremitiesConnected",
                      baseRateTwoToOneExtremitiesConnected);
  if (baseRateTwoToOneExtremitiesConnected < 0.0) {
    throw GeneralException("The parameter baseRateTwoToOneExtremitiesConnected "
                           "contains a wrong value.");
  }

  // Force the (un)binding rates to zero when binding dynamics is turned off,
  // because then the maximum number of full linkers can be properly set before
  if (!bindingDynamics) {
  ...
  if (bindingDynamics && cooperativity) {
    throw GeneralException(
        "Currently, cooperativity is only supported on a system with hopping "
        "crosslinkers, not on a system with binding dynamics.");
  }
  ...
  Propagator propagator(
      numberEquilibrationBlocks, numberRunBlocks, nTimeSteps, calcTimeStep,
      positionProbePeriod, diffusionConstantMicrotubule, springConstant,
      latticeSpacing, ratePassivePartialHop, ratePassiveFullHop,
      baseRateActivePartialHop, baseRateActiveFullHop,
      activeHopToPlusBiasEnergy, neighbourBiasEnergy,
      baseRateZeroToOneExtremitiesConnected,
      baseRateOneToZeroExtremitiesConnected,
      baseRateOneToTwoExtremitiesConnected,
      baseRateTwoToOneExtremitiesConnected, nPassiveCrosslinkers != 0,
      nDualCrosslinkers != 0, nActiveCrosslinkers != 0, headBindingBiasEnergy,
      generator, samplePositionalDistribution, recordTransitionPaths,
      transitionPathProbePeriod, addExternalForce, estimateTimeEvolutionAtPeak,
      log);
  ...
  if (showGraphics && timeStepsDisplayInterval <= 0) {
    throw GeneralException(
        "The parameter timeStepsDisplayInterval contains a wrong value.");
  }
  ...
  if (showGraphics && updateDelayInMilliseconds < 0) {
    throw GeneralException(
        "The parameter updateDelayInMilliseconds contains a wrong value.");
  }
  ...
  propagator.equilibrate(
      systemState, generator,
      output); // run the dynamics for a certain time such that an equilibrium
               // distribution can be reached (not guaranteed to be done)

  if (showGraphics) {
    Graphics graphics(runName, systemState, propagator, generator, output,
                      timeStepsDisplayInterval, updateDelayInMilliseconds);
