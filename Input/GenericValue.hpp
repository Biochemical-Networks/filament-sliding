...
/* A class that can hold a single value of different types. The same can be achieved more efficiently using boost::variant (or std::variant, C++17)
 * However, the choice is made to only use the standard library, disallowing boost::variant, and C++17 is not available yet
 * This class will allow for storing different types in one container (e.g. std::map).
 * The class cannot be a template class with the type of the parameter as the template parameter, because this would not allow for storing different versions in one container
 *
 */
...
private:
    ...
    // Set the default values to null. Only one of the variables is used, the others are garbage.
...
public:
    ...
    /* Use specialized constructors and an overloaded assignment operator to assign values.
     * Are not "explicit", so arguments of functions can implicitly converted to GenericValue using these constructors.
     ...
     * because a unit always needs to be present, and assignment takes one parameter.
     */
     ...
     * This can be done, because the type cannot change after construction.
     * Additionally, the type has to be given as a string upon input.
     */
    friend std::ostream& operator<< (std::ostream& out, const GenericValue &genericValue);
    friend std::istream& operator>> (std::istream& in, GenericValue &genericValue); // not const, needs to change it
