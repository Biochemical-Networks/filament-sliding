...
private:
     ...
     * Store the parameter names (the keys) also in the deque, which is FIFO. Together, it can be printed as a FIFO map.
     */
...
public:
     ...
     * By not using a return type but a reference to the variable it needs to be copied into,
     * the compiler can deduce the template type from the argument type. Hence, no type has to be given calling this function
     */
    template <typename T>
