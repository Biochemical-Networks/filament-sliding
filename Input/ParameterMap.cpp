...
std::ostream& operator<< (std::ostream& out, const ParameterMap &parameterMap)
{
    ...
    for (auto name : parameterMap.m_parameterOrder) // Use parameterOrder to output with the proper order
    {
        ...
        out << parameterMap.m_parameterMap.at(name); // Print the GenericValue that is stored with the name
        out << std::setw(InputFileGeometry::possibleValuesWidth) << parameterMap.m_possibleValuesMap.at(name) << '\n'; // Print the possible values of the parameter
    }
    return out;
}
