...
Input::Input(const std::string fileName) : m_fileName(fileName)
{
    ...
    catch(InputException &inputException) // Catch problems caused by an non-existing or wrongly formatted input file
    {
        try
        {
            produceDefault();
            read(); // Read again, otherwise data that was already read in before encountering a mistake will remain in m_parameterMap
        }
        catch(InputException &inputException) // Catch problems caused by the user not wanting to override the input file, or file permission issues
        {
...
void Input::read()
{
    std::ifstream inputFile(m_fileName.c_str()); // Open and close here and don't make it a member variable, because then the stream would remain open after failing half way through, and you'll read from a certain position

    if (!inputFile)
    {
        throw InputException(std::string("The input file could not be opened")); // Handling the exception can include asking for a default
    }
    else
    {
        inputFile >> m_parameterMap; // Read the file into memory (parameterMap). The operator is overloaded
    }
}

void Input::produceDefault()
{
    if (askMakeDefault()) // returns true if the user wants a default file to be created
    {
        std::ofstream inputFile(m_fileName.c_str());

        if (!inputFile)
        {
            throw InputException(std::string("Program stopped because no input file could be created."));
        }
        else
        {
            inputFile << ParameterMap(); // Create an anonymous object that contains the default values for the parameters, and print it to the file
        }
    }
    else
    {
        throw InputException(std::string("User has stopped the creation of a default input file"));
    }
}

// The only purpose of the function is to ask the user to produce a default input file
bool Input::askMakeDefault()
{
    ...
    while (redo)
    {
        ...
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n'); // Delete all remaining characters in the input stream, without an assumption on the input size
        ...
        else
        {
            ...
            {
                std::cout << "Please enter yes (Y) or no (n). Be aware of capitalisation.\n";
                redo = true;
            }
        }
...
bool Input::fileExists(const std::string& fileName)
{
    ...
    return (stat(fileName.c_str(), &buf) != -1); // stat returns -1 if a file doesn't exist (0 if it does)
}
...
void Input::setRunName()
{
    ...
    {
        std::cout << "\"runName\" was not one of the parameters, so the default run name \"run\" will be used.\n";
        runName = "run";
    }
    int label = 0;
