...
CommandArgumentHandler::CommandArgumentHandler(int argc, char *argv[])
    : m_runNameDefined(false), m_runName(""),
      m_mobileMicrotubuleLengthDefined(false), m_lengthMobile(0.0),
      m_numberOfPassiveCrosslinkersDefined(false), m_numberPassive(0) {
  constexpr int32_t maximumNumberOfArguments =
      1 + 2 * static_cast<int32_t>(VariableName::INVALID);
  if (argc > 1) {
    try {
      // expect an argument declaration and value, so an even number of elements
      // should be in argv[].
      if (argc % 2 != 1 ||
          argc > maximumNumberOfArguments) // the name of the program is also
                                           // counted by argc
      {
        std::cerr << "The structure of the command line arguments is not "
                     "recognised, so these will be ignored and the program "
        ...
        for (int32_t i = 0; i != numberSetArguments; ++i) {
          ...
          readVariable(std::istringstream{argv[2 * i + 1]},
...
void CommandArgumentHandler::readVariable(std::istringstream &&streamName,
  ...
  if (variableType == "-N" || variableType == "-n") {
    ...
    throw InputException("CommandArgumentHandler::readVariable() could not "
  ...
  case VariableName::RUNNAME:
    if (m_runNameDefined) {
      throw InputException("CommandArgumentHandler::readVariable() tried to "
                           "set the run name more than once.");
    }
    if (!(streamValue >> m_runName)) {
      throw InputException("CommandArgumentHandler::readVariable() did not "
  ...
  case VariableName::MOBILELENGTH:
    ...
    if (!(streamValue >> m_lengthMobile)) {
      throw InputException(
          "CommandArgumentHandler::readVariable() did not encounter a proper "
  ...
  default:
    throw InputException("CommandArgumentHandler::readVariable() set "
...
#ifdef MYDEBUG
  if (!m_runNameDefined) {
    throw InputException("CommandArgumentHandler::getRunName() was called when "
...
#ifdef MYDEBUG
  if (!m_mobileMicrotubuleLengthDefined) {
    throw InputException("CommandArgumentHandler::getLength() was called when "
...
int32_t CommandArgumentHandler::getNumberPassive() const {
  ...
  if (!m_numberOfPassiveCrosslinkersDefined) {
    throw InputException(
        "CommandArgumentHandler::getNumberPassive() was called when the "
