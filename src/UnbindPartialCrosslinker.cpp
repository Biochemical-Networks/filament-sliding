...
void UnbindPartialCrosslinker::setCurrentRate(const SystemState& systemState) {
    ...
    if (nCrosslinkersOfThisType !=
        nPartialsBoundWithHeadAndTail.first +
                nPartialsBoundWithHeadAndTail.second) {
        throw GeneralException(
                "UnbindPartialCrosslinker::setCurrentRate() found two "
                "different numbers of partial linkers");
    }
#endif // MYDEBUG
...
void UnbindPartialCrosslinker::performReaction(
...
Crosslinker& UnbindPartialCrosslinker::whichToDisconnect(
        SystemState& systemState,
    ...
    if (partialsBoundWithHead.empty() && partialsBoundWithTail.empty()) {
        throw GeneralException(
                "No partial linker is available to be disconnected in "
                "UnbindPartialCrosslinker::whichToDisconnect()");
    }
#endif // MYDEBUG
    ...
    // Choose from which of the two sets (bound with head or tail) the
    // crosslinker needs to be picked.
