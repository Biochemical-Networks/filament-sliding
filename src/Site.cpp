...
void Site::connectCrosslinker(Crosslinker &crosslinkerToConnect,
                              const Crosslinker::Terminus terminusToConnect) {
#ifdef MYDEBUG
  if (!m_isFree) {
    throw GeneralException("Site::connectCrosslinker() tried to connect a "
                           "crosslinker that was already connected");
  }
#endif // MYDEBUG

  mp_connectedCrosslinker =
      &crosslinkerToConnect; // Store the address of the connected crosslinker
...
#ifdef MYDEBUG
  if (m_isFree) {
    throw GeneralException("Site::disconnectCrosslinker() tried to disconnect "
                           "a crosslinker that was not connected");
  }
#endif // MYDEBUG
...
Crosslinker *Site::whichCrosslinkerIsBound() const {
  ...
  if (m_isFree) {
    throw GeneralException(
        "Site::whichCrosslinkerIsBound() was called on a free Crosslinker");
  }
#endif // MYDEBUG
