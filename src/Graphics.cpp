...
Graphics::Graphics(
        ...
        m_trueLatticeSpacing(
                static_cast<float>(systemState.getLatticeSpacing())),
        ...
        m_mobileMicrotubule(
                systemState.getNSites(MicrotubuleType::MOBILE),
                m_circleRadius,
                m_lineLength,
                m_lineThickness,
                m_circlePointCount),
        m_fixedMicrotubule(
                systemState.getNSites(MicrotubuleType::FIXED),
                m_circleRadius,
                m_lineLength,
                m_lineThickness,
                m_circlePointCount),
    ...
    const float centreOverlap = static_cast<float>(
                                        (systemState.beginningOverlap() +
                                         0.5 * systemState.overlapLength()) /
    ...
    // Reserve the space once, since this may prevent many initial
    // reallocations. However, after these calls, the vectors change capacity
    // automatically
...
void Graphics::performMainLoop() {
    ...
    while (m_window.isOpen()) {
        ...
        if (!m_pause && clock.getElapsedTime() > m_updateDelay) {
            // Restart the clock before the update: the time to propagate should
            // be part of the delay, not extra
            clock.restart();
...
void Graphics::handleContinuousKeyPresses() {
    ...
    if (zoomIn && !zoomOut) {
        ...
        m_view.zoom(zoomFactor); // the arrows work much faster than the
                                 // scrolling, therefore we use the factor 0.1
        m_window.setView(m_view);
    }
...
void Graphics::handleEvent(const sf::Event& event) {
    if (event.type == sf::Event::Closed) {
        ...
        const float centreOverlap =
                static_cast<float>(
                        (m_systemState.beginningOverlap() +
        ...
        const double zoomFactor = std::max(
                1.f - event.mouseWheelScroll.delta * m_scrollStepSize,
                m_scrollStepSize);
        m_currentZoomFactor *= zoomFactor;
...
void Graphics::update() {
    ...
    m_mobileMicrotubule.setPosition(
            calculateMobileMicrotubuleX(), calculateMobileMicrotubuleY());
    m_fixedMicrotubule.setPosition(
            m_fixedMicrotubuleX, calculateFixedMicrotubuleY());
...
void Graphics::propagateGraphicsStep() {
    m_propagator.propagateGraphicsInterval(
            m_systemState, m_generator, m_output, m_timeStepsDisplayInterval);
...
void Graphics::updatePartialCrosslinkers(const Crosslinker::Type type) {
    for (Crosslinker* p_linker: m_systemState.getPartialLinkers(type)) {
        ...
        case Crosslinker::Type::DUAL:
            boundWithMotor =
                    (p_linker->getBoundTerminusWhenPartiallyConnected() ==
        ...
        default:
            throw GeneralException(
                    "Graphics::updatePartialCrosslinkers() "
        ...
        m_partialCrosslinkers.push_back(PartialCrosslinkerGraphic(
                m_circleRadius - m_lineThickness,
                m_lineThickness,
        ...
        if (boundLocation.microtubule == MicrotubuleType::FIXED) {
            ...
            m_partialCrosslinkers.back().setPosition(
                    m_fixedMicrotubuleX + calculateGraphicsLatticeSpacing() *
                                                  boundLocation.position,
...
void Graphics::updateFullCrosslinkers(const Crosslinker::Type type) {
    for (Crosslinker* p_linker: m_systemState.getFullLinkers(type)) {
        ...
        default:
            throw GeneralException(
                    "Graphics::updateFullCrosslinkers() encountered a "
                    "wrong linker type.");
            break;
        }
        ...
        if (headLocation.microtubule == MicrotubuleType::FIXED) {
            fixedPosition = sf::Vector2f(
                    m_fixedMicrotubuleX + calculateGraphicsLatticeSpacing() *
                                                  headLocation.position,
                    calculateFixedMicrotubuleY());
            mobilePosition = sf::Vector2f(
                    calculateMobileMicrotubuleX() +
                            calculateGraphicsLatticeSpacing() *
