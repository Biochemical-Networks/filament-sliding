...
#include "filament-sliding/Crosslinker.hpp" // For Crosslinker::Type
...
Graphics::Graphics(const std::string &runName, SystemState &systemState,
                   Propagator &propagator, RandomGenerator &generator,
                   Output &output, const int32_t timeStepsDisplayInterval,
                   const int32_t updateDelayInMilliseconds)
    : m_trueLatticeSpacing(static_cast<float>(systemState.getLatticeSpacing())),
      m_trueInitialPosition(
          static_cast<float>(systemState.getMicrotubulePosition())),
      m_fixedMicrotubuleX(m_screenBorderThickness), m_lineLength(50.f),
      m_distanceBetweenMicrotubules(200.f), m_systemState(systemState),
      m_propagator(propagator), m_generator(generator), m_output(output),
      ...
      m_mobileMicrotubule(systemState.getNSites(MicrotubuleType::MOBILE),
                          m_circleRadius, m_lineLength, m_lineThickness,
                          m_circlePointCount),
      m_fixedMicrotubule(systemState.getNSites(MicrotubuleType::FIXED),
                         m_circleRadius, m_lineLength, m_lineThickness,
                         m_circlePointCount),
      m_currentZoomFactor(1.0) {
  // const float worldWidth = std::max({static_cast<float>(m_windowWidth),
  //                                     systemState.getNSites(MicrotubuleType::MOBILE)*calculateGraphicsLatticeSpacing()
  //                                     + 2*m_screenBorderThickness,
  //                                     systemState.getNSites(MicrotubuleType::FIXED)*calculateGraphicsLatticeSpacing()
  //                                     + 2*m_screenBorderThickness});

  const float centreOverlap =
      static_cast<float>(
          (systemState.beginningOverlap() + 0.5 * systemState.overlapLength()) /
          systemState.getLatticeSpacing()) *
      calculateGraphicsLatticeSpacing();

  m_contextSettings.antialiasingLevel = 8;

  m_window.create(
      sf::VideoMode(m_windowWidth, m_windowHeight), "Crosslink: " + runName,
      sf::Style::Default,
      m_contextSettings); // The window title is "Crosslink: "+runName
  ...
  update(); // Set all graphical entities, including the microtubules, at the
            // right positions etc.

  // Reserve the space once, since this may prevent many initial reallocations.
  // However, after these calls, the vectors change capacity automatically
  m_partialCrosslinkers.reserve(
      systemState.getPartialLinkers(Crosslinker::Type::PASSIVE).size() +
      systemState.getPartialLinkers(Crosslinker::Type::DUAL).size() +
      systemState.getPartialLinkers(Crosslinker::Type::ACTIVE).size());
  m_fullCrosslinkers.reserve(
      systemState.getFullLinkers(Crosslinker::Type::PASSIVE).size() +
...
void Graphics::performMainLoop() {
  ...
  while (m_window.isOpen()) {
    ...
    if (!m_pause && clock.getElapsedTime() > m_updateDelay) {
      // Restart the clock before the update: the time to propagate should be
      // part of the delay, not extra
      clock.restart();
...
void Graphics::handleContinuousKeyPresses() {
  ...
  const bool zoomIn = sf::Keyboard::isKeyPressed(sf::Keyboard::Up) ||
                      sf::Keyboard::isKeyPressed(sf::Keyboard::W);
  const bool zoomOut = sf::Keyboard::isKeyPressed(sf::Keyboard::Down) ||
                       sf::Keyboard::isKeyPressed(sf::Keyboard::S);

  if (zoomIn && !zoomOut) {
    const double zoomFactor = 1.f - m_keyPressedScaleStep;
    m_currentZoomFactor *= zoomFactor;
    m_view.zoom(zoomFactor); // the arrows work much faster than the scrolling,
                             // therefore we use the factor 0.1
    m_window.setView(m_view);
  ...
  const bool increaseLatticeSpacing =
      sf::Keyboard::isKeyPressed(sf::Keyboard::PageUp);
  const bool decreaseLatticeSpacing =
...
void Graphics::handleEvent(const sf::Event &event) {
  if (event.type == sf::Event::Closed) {
    ...
    const float centreOverlap =
        static_cast<float>((m_systemState.beginningOverlap() +
                            0.5 * m_systemState.overlapLength()) /
                           m_systemState.getLatticeSpacing()) *
        calculateGraphicsLatticeSpacing();
    ...
    const double zoomFactor =
        std::max(1.f - event.mouseWheelScroll.delta * m_scrollStepSize,
                 m_scrollStepSize);
    m_currentZoomFactor *= zoomFactor;
...
void Graphics::update() {
  ...
  m_mobileMicrotubule.setPosition(calculateMobileMicrotubuleX(),
                                  calculateMobileMicrotubuleY());
  m_fixedMicrotubule.setPosition(m_fixedMicrotubuleX,
                                 calculateFixedMicrotubuleY());
...
void Graphics::propagateGraphicsStep() {
  m_propagator.propagateGraphicsInterval(m_systemState, m_generator, m_output,
                                         m_timeStepsDisplayInterval);
...
void Graphics::updatePartialCrosslinkers(const Crosslinker::Type type) {
  for (Crosslinker *p_linker : m_systemState.getPartialLinkers(type)) {
    ...
    case Crosslinker::Type::DUAL:
      boundWithMotor = (p_linker->getBoundTerminusWhenPartiallyConnected() ==
    ...
    case Crosslinker::Type::ACTIVE:
      ...
      break;
    default:
      throw GeneralException("Graphics::updatePartialCrosslinkers() "
                             "encountered a wrong linker type.");
      break;
    }

    const SiteLocation boundLocation =
        p_linker->getBoundLocationWhenPartiallyConnected();
    m_partialCrosslinkers.push_back(PartialCrosslinkerGraphic(
        m_circleRadius - m_lineThickness, m_lineThickness,
        0.5f * m_distanceBetweenMicrotubules, boundWithMotor,
        m_circlePointCount));

    if (boundLocation.microtubule == MicrotubuleType::FIXED) {
      m_partialCrosslinkers.back().rotate(
          180.f); // by default, it is pointing downwards
      m_partialCrosslinkers.back().setPosition(
          m_fixedMicrotubuleX +
              calculateGraphicsLatticeSpacing() * boundLocation.position,
          calculateFixedMicrotubuleY());
    } else {
      m_partialCrosslinkers.back().setPosition(
          calculateMobileMicrotubuleX() +
              calculateGraphicsLatticeSpacing() * boundLocation.position,
          calculateMobileMicrotubuleY());
    }
  }
  ...
  for (Crosslinker *p_linker : m_systemState.getFullLinkers(type)) {
    ...
    case Crosslinker::Type::DUAL:
      mobileTerminusActive =
          (p_linker->getTerminusOfFullOn(MicrotubuleType::MOBILE) ==
           Crosslinker::Terminus::HEAD);
    ...
    case Crosslinker::Type::ACTIVE:
      ...
      break;
    default:
      throw GeneralException("Graphics::updateFullCrosslinkers() encountered a "
                             "wrong linker type.");
      break;
    }

    const SiteLocation headLocation =
        p_linker->getOneBoundLocationWhenFullyConnected(
            Crosslinker::Terminus::HEAD);
    const SiteLocation tailLocation =
        p_linker->getOneBoundLocationWhenFullyConnected(
    ...
    if (headLocation.microtubule == MicrotubuleType::FIXED) {
      fixedPosition =
          sf::Vector2f(m_fixedMicrotubuleX + calculateGraphicsLatticeSpacing() *
                                                 headLocation.position,
                       calculateFixedMicrotubuleY());
      mobilePosition = sf::Vector2f(calculateMobileMicrotubuleX() +
                                        calculateGraphicsLatticeSpacing() *
                                            tailLocation.position,
                                    calculateMobileMicrotubuleY());
    } else {
      fixedPosition =
          sf::Vector2f(m_fixedMicrotubuleX + calculateGraphicsLatticeSpacing() *
    ...
    m_fullCrosslinkers.push_back(FullCrosslinkerGraphic(
        m_circleRadius - m_lineThickness, m_lineThickness, mobileTerminusActive,
        fixedTerminusActive, mobilePosition, fixedPosition,
        m_circlePointCount));
  }
}
...
float Graphics::calculateMobileMicrotubuleX() const {
  return static_cast<float>(m_systemState.getMicrotubulePosition()) /
             m_trueLatticeSpacing * calculateGraphicsLatticeSpacing() +
         m_screenBorderThickness;
}

float Graphics::calculateMobileMicrotubuleY() const {
  return (static_cast<float>(m_windowHeight) - m_distanceBetweenMicrotubules) *
         0.5f;
}
