...
void Graphics::handleContinuousKeyPresses() {
    ...
    if (zoomIn && !zoomOut) {
        ...
        m_view.zoom(
                1.f - m_keyPressedScaleStep); // the arrows work much faster
                                              // than the scrolling, therefore
                                              // we use the factor 0.1
        m_window.setView(m_view);
    }
    ...
    for (Crosslinker* p_linker: m_systemState.getPartialLinkers(type)) {
        ...
        default:
            throw GeneralException(
                    "Graphics::updatePartialCrosslinkers() encountered a wrong "
                    "linker type.");
            break;
        }
...
void Graphics::updateFullCrosslinkers(const Crosslinker::Type type) {
    for (Crosslinker* p_linker: m_systemState.getFullLinkers(type)) {
        ...
        default:
            throw GeneralException(
                    "Graphics::updateFullCrosslinkers() encountered a wrong "
                    "linker type.");
            break;
        }
