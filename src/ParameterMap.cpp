...
std::ostream &operator<<(std::ostream &out, const ParameterMap &parameterMap) {
  ...
  out << std::setw(InputFileGeometry::nameWidth) << "NAME"
      ...
      << std::setw(InputFileGeometry::possibleValuesWidth) << "POSSIBLE_VALUES"
      << '\n';
...
std::istream &operator>>(std::istream &in, ParameterMap &parameterMap) {
                                      ...
                                      // This could be changed
  {
    ...
    if (readName != parameterName) {
      throw InputException("The parameter " + readName +
                           " is not recognized as a valid parameter or the "
                           "parameters are not ordered correctly");
    }
    ...
    if (possibleValues !=
        ParameterMap().m_possibleValuesMap.at(
            parameterName)) // Check if the default ParameterMap contains the
                            // same possibleValues as the one read
    {
      throw InputException("The parameter " + readName +
                           " does not have correct allowed values, or the "
                           "parameters are not ordered correctly");
    }
