...
PartialCrosslinkerGraphic::PartialCrosslinkerGraphic(
    ...
    const float springLength, const bool boundTerminusActive,
    const std::size_t circlePointCount)
    : m_circleRadius(circleRadius), m_lineThickness(lineThickness),
      m_springLength(springLength), m_terminus(circleRadius, circlePointCount),
      m_spring(sf::Vector2f(lineThickness,
                            springLength)) // A (thin) bar with four corners
{
  m_terminus.setOrigin(circleRadius, circleRadius);
  m_terminus.setFillColor(boundTerminusActive ? m_activeTerminusColor
...
void PartialCrosslinkerGraphic::draw(sf::RenderTarget &target,
