...
Propagator::Propagator(
    const int32_t numberEquilibrationBlocks, const int32_t numberRunBlocks,
    const int32_t nTimeSteps, const double calcTimeStep,
    const int32_t positionProbePeriod,
    const double diffusionConstantMicrotubule, const double springConstant,
    const double latticeSpacing, const double ratePassivePartialHop,
    const double ratePassiveFullHop, const double baseRateActivePartialHop,
    const double baseRateActiveFullHop, const double activeHopToPlusBiasEnergy,
    ...
    const bool bindPassiveLinkers, const bool bindDualLinkers,
    const bool bindActiveLinkers, const double headBindingBiasEnergy,
    RandomGenerator &generator, const bool samplePositionalDistribution,
    const bool recordTransitionPaths, const int32_t transitionPathProbePeriod,
    const bool addExternalForce, const bool estimateTimeEvolutionAtPeak,
    Log &log)
    : m_nEquilibrationBlocks(numberEquilibrationBlocks),
      m_nRunBlocks(numberRunBlocks), m_nTimeSteps(nTimeSteps),
      m_calcTimeStep(calcTimeStep), m_positionProbePeriod(positionProbePeriod),
      m_diffusionConstantMicrotubule(diffusionConstantMicrotubule),
      m_springConstant(springConstant), m_latticeSpacing(latticeSpacing),
      m_deviationMicrotubule(
          std::sqrt(2 * m_diffusionConstantMicrotubule * m_calcTimeStep)),
      m_currentTime(-m_nEquilibrationBlocks * m_nTimeSteps *
      ...
      m_nDeterministicBoundaryCrossings(
          0), // Counts the number of times a force has tried to push the mobile
              // microtubule across a maximum stretch barrier
      m_nStochasticBoundaryCrossings(
          0), // Counts the number of times diffusion of the mobile microtubule
              // was reflected at a maximum stretch barrier
      m_log(log), m_basinOfAttractionHalfWidth(0.3 * m_latticeSpacing) {
  // If no active/dual/partial linkers were set (their number is zero), then set
  // the binding rate to zero.
  ...
  // objects in std::initializer_list are inherently const, so std::unique_ptr's
  // copy constructor cannot be used there, and we cannot use this method of
  // initialising m_reactions. See
  ...
  m_reactions["bindingFreePassiveCrosslinker"] = std::unique_ptr<Reaction>(
      new BindFreeCrosslinker(rateToOneSitePassive, Crosslinker::Type::PASSIVE,
                              headBindingBiasEnergy));
  m_reactions["bindingFreeDualCrosslinker"] =
      std::unique_ptr<Reaction>(new BindFreeCrosslinker(
          rateToOneSiteDual, Crosslinker::Type::DUAL, headBindingBiasEnergy));
  m_reactions["bindingFreeActiveCrosslinker"] = std::unique_ptr<Reaction>(
      new BindFreeCrosslinker(rateToOneSiteActive, Crosslinker::Type::ACTIVE,
                              headBindingBiasEnergy));
  m_reactions["bindingPartialPassiveCrosslinker"] =
      std::unique_ptr<Reaction>(new BindPartialCrosslinker(
          baseRateOneToTwoExtremitiesConnected, Crosslinker::Type::PASSIVE,
          headBindingBiasEnergy, m_springConstant));
  m_reactions["bindingPartialDualCrosslinker"] =
      std::unique_ptr<Reaction>(new BindPartialCrosslinker(
          baseRateOneToTwoExtremitiesConnected, Crosslinker::Type::DUAL,
          headBindingBiasEnergy, m_springConstant));
  m_reactions["bindingPartialActiveCrosslinker"] =
      std::unique_ptr<Reaction>(new BindPartialCrosslinker(
          baseRateOneToTwoExtremitiesConnected, Crosslinker::Type::ACTIVE,
          headBindingBiasEnergy, m_springConstant));
  m_reactions["unbindingPartialPassiveCrosslinker"] =
      std::unique_ptr<Reaction>(new UnbindPartialCrosslinker(
          baseRateOneToZeroExtremitiesConnected, Crosslinker::Type::PASSIVE,
          headBindingBiasEnergy));
  m_reactions["unbindingPartialDualCrosslinker"] =
      std::unique_ptr<Reaction>(new UnbindPartialCrosslinker(
          baseRateOneToZeroExtremitiesConnected, Crosslinker::Type::DUAL,
          headBindingBiasEnergy));
  m_reactions["unbindingPartialActiveCrosslinker"] =
      std::unique_ptr<Reaction>(new UnbindPartialCrosslinker(
          baseRateOneToZeroExtremitiesConnected, Crosslinker::Type::ACTIVE,
          headBindingBiasEnergy));
  m_reactions["unbindingFullPassiveCrosslinker"] =
      std::unique_ptr<Reaction>(new UnbindFullCrosslinker(
          baseRateTwoToOneExtremitiesConnected, Crosslinker::Type::PASSIVE,
          headBindingBiasEnergy, m_springConstant));
  m_reactions["unbindingFullDualCrosslinker"] =
      std::unique_ptr<Reaction>(new UnbindFullCrosslinker(
          baseRateTwoToOneExtremitiesConnected, Crosslinker::Type::DUAL,
          headBindingBiasEnergy, m_springConstant));
  m_reactions["unbindingFullActiveCrosslinker"] =
      std::unique_ptr<Reaction>(new UnbindFullCrosslinker(
          baseRateTwoToOneExtremitiesConnected, Crosslinker::Type::ACTIVE,
          headBindingBiasEnergy, m_springConstant));
  m_reactions["hoppingPartialPassiveCrosslinker"] = std::unique_ptr<Reaction>(
      new HopPartial(ratePassivePartialHop, ratePassivePartialHop,
                     Crosslinker::Type::PASSIVE, 0.0, 0.0,
                     neighbourBiasEnergy)); // For a passive linker, the bias
                                            // energy is zero
  m_reactions["hoppingPartialDualCrosslinker"] = std::unique_ptr<Reaction>(
      new HopPartial(baseRateActivePartialHop, ratePassivePartialHop,
                     Crosslinker::Type::DUAL, activeHopToPlusBiasEnergy, 0.0,
                     neighbourBiasEnergy));
  m_reactions["hoppingPartialActiveCrosslinker"] = std::unique_ptr<Reaction>(
      new HopPartial(baseRateActivePartialHop, baseRateActivePartialHop,
                     Crosslinker::Type::ACTIVE, activeHopToPlusBiasEnergy,
                     activeHopToPlusBiasEnergy, neighbourBiasEnergy));
  m_reactions["hoppingFullPassiveCrosslinker"] =
      std::unique_ptr<Reaction>(new HopFull(
          ratePassiveFullHop, ratePassiveFullHop, Crosslinker::Type::PASSIVE,
          m_springConstant, 0.0, 0.0, neighbourBiasEnergy));
  m_reactions["hoppingFullDualCrosslinker"] = std::unique_ptr<Reaction>(
      new HopFull(baseRateActiveFullHop, ratePassiveFullHop,
                  Crosslinker::Type::DUAL, m_springConstant,
                  activeHopToPlusBiasEnergy, 0.0, neighbourBiasEnergy));
  m_reactions["hoppingFullActiveCrosslinker"] = std::unique_ptr<Reaction>(
      new HopFull(baseRateActiveFullHop, baseRateActiveFullHop,
                  Crosslinker::Type::ACTIVE, m_springConstant,
                  activeHopToPlusBiasEnergy, activeHopToPlusBiasEnergy,
                  neighbourBiasEnergy));

  // The standard deviation of the average microtubule position update should be
  // much smaller (orders of magnitude smaller) than the lattice spacing, since
  // that sets a scale over which force differences definitely emerge. The
  // choice 0.1 is pretty large, but this is a hard maximum limit.
  if (m_deviationMicrotubule > (0.1 * m_latticeSpacing)) {
    throw GeneralException(
        "The time step is too large to allow for the approximate microtubule "
  ...
  // The reaction rate is integrated discretely over the time steps, and leads
  // to a reaction when this accumulated reaction rate (or action) comes above a
  // certain threshold. The threshold is set by inverting the survival
...
void Propagator::propagateBlock(SystemState &systemState,
                                RandomGenerator &generator, Output &output,
  ...
  for (int32_t timeStep = 0; timeStep < nTimeSteps; ++timeStep) {
    if (writeOutput) {
      if (timeStep % m_positionProbePeriod == 0) {
        output.writeMicrotubulePosition(
            m_currentTime,
            systemState); // writes the position and the number of crosslinkers
                          // (the order parameters)
      }
      // Add the microtubule positions more often than the
      // m_positionProbePeriod, since it is not directly written to a file (not
      // slow), and it requires much data.
      if (m_samplePositionalDistribution) {
        output.addPositionAndConfiguration(
            MathematicalFunctions::mod(systemState.getMicrotubulePosition(),
                                       m_latticeSpacing),
      ...
      if (m_recordTransitionPaths) {
        ...
        if (!inBasinOfAttraction(position, nRightLinkers, nFullLinkers)) {
          if (!output.isTrackingPath()) {
            ...
            // At the first m_transitionPathProbePeriod out of the basin, it
            // should still be close to the original basin.
            m_previousBasinOfAttraction = MathematicalFunctions::intFloor(
        ...
        {
          if (output.isTrackingPath()) {
            if (MathematicalFunctions::intFloor(position / m_latticeSpacing +
                                                0.5) ==
              ...
              // m_transitionPathProbePeriod is too small to probe a transition
              // path effectively
            }
            output.toggleTracking(); // Turn off at the end, after the possible
    ...
    const int32_t barrierCrossingDirection =
        systemState.barrierCrossed(); // returns 0 if no barrier is crossed
    if (barrierCrossingDirection != 0) {
      if (writeOutput) {
        output.writeBarrierCrossingTime(m_currentTime,
                                        barrierCrossingDirection);
      }
    }
  }
}

bool Propagator::inBasinOfAttraction(const double mobilePosition,
...
void Propagator::equilibrate(SystemState &systemState,
                             RandomGenerator &generator, Output &output) {
...
void Propagator::run(SystemState &systemState, RandomGenerator &generator,
                     Output &output) {
...
void Propagator::propagateGraphicsInterval(SystemState &systemState,
...
void Propagator::advanceTimeStep(SystemState &systemState,
                                 RandomGenerator &generator) {
  // First, update the reaction rates and actions, and perform a reaction when
  // the total action surpasses the threshold. Then, move the mobile microtubule
  // at the end of the time step
  setRates(systemState);
  updateAction();
  if (getTotalAction() > m_currentReactionRateThreshold) {
    performReaction(systemState,
...
void Propagator::moveMicrotubule(SystemState &systemState,
                                 RandomGenerator &generator) {
  ...
  // function (see notes). The change c follows the formula dc/dt = -k D (E0 + N
  // c). Here, k is the spring constant (in units of k_B T), D is the diffusion
  // constant (and thus the mobility through the Einstein relation), E0 is the
  // totalExtension at time 0, and N is the number of crosslinkers. The change
  // in extension upon a change c is N*c. which for small time steps reduces to:
  // m_diffusionConstantMicrotubule*systemState.getForce()*m_calcTimeStep
  // use the expm1 function to prevent catastrophic cancellation for very small
  // numbers. Check for the case of no crosslinkers. The numberFullLinkers is
  ...
  double deterministicChange =
      (numberFullLinkers != 0)
          ? ((totalExtension -
             ...
             numberFullLinkers *
             std::expm1(-numberFullLinkers * m_springConstant *
                        m_diffusionConstantMicrotubule * m_calcTimeStep))
          : (systemState.findExternalForce() * m_diffusionConstantMicrotubule *
             m_calcTimeStep);

  // Check if the deterministic change is breaking the boundaries; if so: place
  // the particle just on the proper side of the boundary
  if (deterministicChange <= exclusiveMovementBorders.first) {
    deterministicChange = std::nextafter(exclusiveMovementBorders.first,
                                         exclusiveMovementBorders.second);
    ++m_nDeterministicBoundaryCrossings;
  } else if (deterministicChange >= exclusiveMovementBorders.second) {
    deterministicChange = std::nextafter(exclusiveMovementBorders.second,
  ...
  if (deterministicChange <= exclusiveMovementBorders.first ||
    ...
    throw GeneralException(
        "In Propagator::moveMicrotubule(), the deterministic change was "
        "outside of the allowed zone.");
  }
#endif // MYDEBUG

  // update the exclusiveMovementBorders, such that they are now relative to the
  // new position after the deterministic change
  exclusiveMovementBorders.first -= deterministicChange;
  exclusiveMovementBorders.second -= deterministicChange;

  // Use a reflecting boundary condition for the random change. First, calculate
  // the Gaussian change.
  ...
  // The while loop is there to check if a double reflection is necessary (this
  // should really never happen, so while acts as an if here)
...
void Propagator::performReaction(SystemState &systemState,
                                 RandomGenerator &generator) {
...
void Propagator::setNewReactionRateThreshold(const double probability) {
  ...
  // If probability is in the open interval (0,1), then the threshold is in the
  // range (0, infty)
...
Reaction &Propagator::getReactionToHappen(RandomGenerator &generator) const {
  const double randomNumber =
      generator.getUniform(0.0, getTotalRate()); // uses interval [0,totalRate)

  double accumulatedRate = 0.0;
  for (const auto &reaction : m_reactions) {
    // Calculate the rate in the same way as in getTotalRate(), such that the
    // double precision numbers will be equal
    accumulatedRate += reaction.second->getCurrentRate();

    // If the randomNumber falls exactly on the border between two reactions,
    // the later one is chosen. Since 0 is an option, this does not bias any
    // reaction
