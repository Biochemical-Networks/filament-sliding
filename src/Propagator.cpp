...
Propagator::Propagator(
    ...
    m_reactions["hoppingPartialPassiveCrosslinker"] =
            std::unique_ptr<Reaction>(new HopPartial(
                    ...
                    Crosslinker::Type::PASSIVE,
                    0.0,
                    0.0)); // For a passive linker, the bias energy is zero
    ...
    if (m_deviationMicrotubule > (0.1 * m_latticeSpacing)) {
        throw GeneralException(
                ...
                "microtubule movement to be trustworthy. See Propagator "
                "constructor");
    }
    ...
    for (int32_t timeStep = 0; timeStep < nTimeSteps; ++timeStep) {
        if (writeOutput) {
            ...
            if (m_samplePositionalDistribution) {
                output.addMicrotubulePositionRemainder(
...
void Propagator::advanceTimeStep(
    ...
    if (getTotalAction() > m_currentReactionRateThreshold) {
        performReaction(
                systemState, generator); // also updates the force and action
    }
...
void Propagator::moveMicrotubule(
    ...
    const double numberFullLinkers =
            static_cast<double>(systemState.getNFullCrosslinkers());
    // The integrated change is given by the following (exponential minus one)
    // function (see notes), which for small time steps reduces to:
    ...
    double deterministicChange =
            totalExtension / numberFullLinkers *
            std::expm1(
                    -numberFullLinkers * m_springConstant *
                    m_diffusionConstantMicrotubule * m_calcTimeStep);

    if (m_addTheoreticalCounterForce) {
        deterministicChange += m_diffusionConstantMicrotubule *
                               systemState.findExternalForce() * m_calcTimeStep;
    }
    ...
    if (deterministicChange <= exclusiveMovementBorders.first ||
        ...
        throw GeneralException(
                ...
                "was outside of the allowed zone.");
    }
#endif // MYDEBUG
...
Reaction& Propagator::getReactionToHappen(RandomGenerator& generator) const {
    double randomNumber = generator.getUniform(
    ...
    for (const auto& reaction: m_reactions) {
        ...
        // not bias any reaction randomNumber is strictly smaller than the upper
        // bound totalAccumulatedAction, so subtracting the total upper bound
        // after the for loop will definitely make it lower than 0.0
        if (randomNumber < 0.0) {
