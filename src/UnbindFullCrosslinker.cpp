...
UnbindFullCrosslinker::UnbindFullCrosslinker(
    const double rateOneLinkerUnbinds, const Crosslinker::Type typeToUnbind,
    const double headBiasEnergy, const double springConstant)
    : Reaction(), m_rateOneLinkerUnbinds(rateOneLinkerUnbinds),
      m_typeToUnbind(typeToUnbind),
      m_probHeadUnbinds(
          1 / (1 + std::exp(-headBiasEnergy))), // headBiasEnergy should have
                                                // units of (k_B T)
...
void UnbindFullCrosslinker::setCurrentRate(const SystemState &systemState) {
  const std::vector<FullConnection> &fullConnections =
      systemState.getFullConnections(m_typeToUnbind);
  ...
  for (const FullConnection &fullConnection : fullConnections) {
    ...
    // rate scales with exp(k x^2 / (4 k_B T)) Give the rate of unbinding this
    // crosslinker: which terminus is unbound is decided upon performing the
    // actual event.
    const double rate = m_rateOneLinkerUnbinds *
                        std::exp(m_springConstant * fullConnection.extension *
                                 fullConnection.extension * 0.25);
...
void UnbindFullCrosslinker::performReaction(SystemState &systemState,
                                            RandomGenerator &generator) {
  FullConnection connectionToDisconnect =
      whichToDisconnect(systemState, generator);

  Crosslinker::Terminus terminusToDisconnect =
      ((generator.getBernoulli(m_probHeadUnbinds))
           ? (Crosslinker::Terminus::HEAD)
           : (Crosslinker::Terminus::TAIL));

  systemState.disconnectFullyConnectedCrosslinker(
      *connectionToDisconnect.p_fullLinker, terminusToDisconnect);
}

FullConnection
UnbindFullCrosslinker::whichToDisconnect(SystemState &systemState,
  ...
  if (fullConnections.empty()) {
    throw GeneralException("UnbindFullCrosslinker::whichToDisconnect() was not "
                           "able to disconnect a linker");
  }
  if (fullConnections.size() != m_individualRates.size()) {
    throw GeneralException("UnbindFullCrosslinker::whichToDisconnect() was "
  ...
  throw GeneralException(
      "The end of UnbindFullCrosslinker::whichToConnect() was reached");
}
