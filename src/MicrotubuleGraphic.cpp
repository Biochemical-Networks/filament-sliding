                                             ...
                                             // is 4; nSites-1 quads are needed
{
    for (int32_t pos = 0; pos < m_nSites; ++pos) {
        m_circles[pos].setOrigin(
                m_circleRadius, m_circleRadius); // set the origins of the
                                                 // circles to their centre
        m_circles[pos].setFillColor(m_fillColor);
        m_circles[pos].setOutlineThickness(
                -m_lineThickness); // A negative value means that the outline
                                   // goes inward, and the radius stays the same
        m_circles[pos].setOutlineColor(m_microtubuleColor);
        m_circles[pos].setPosition(
                pos * (m_lineLength + 2 * m_circleRadius), 0.f);
    }
    for (int32_t pos = 0; pos < m_nSites - 1;
         ++pos) // The number of lines equals nSites-1
    {
        m_lines[4 * pos + 0].position = sf::Vector2f(
                pos * (m_lineLength + 2 * m_circleRadius) + m_circleRadius -
                        0.5f * m_lineThickness,
                0.5f * m_lineThickness);
        m_lines[4 * pos + 0].color = m_microtubuleColor;
        m_lines[4 * pos + 1].position = sf::Vector2f(
                pos * (m_lineLength + 2 * m_circleRadius) + m_circleRadius +
                        m_lineLength + 0.5f * m_lineThickness,
                0.5f * m_lineThickness);
        m_lines[4 * pos + 1].color = m_microtubuleColor;
        m_lines[4 * pos + 2].position = sf::Vector2f(
                pos * (m_lineLength + 2 * m_circleRadius) + m_circleRadius +
                        m_lineLength + 0.5f * m_lineThickness,
                -0.5f * m_lineThickness);
        m_lines[4 * pos + 2].color = m_microtubuleColor;
        m_lines[4 * pos + 3].position = sf::Vector2f(
                pos * (m_lineLength + 2 * m_circleRadius) + m_circleRadius -
                        0.5f * m_lineThickness,
                -0.5f * m_lineThickness);
...
void MicrotubuleGraphic::draw(sf::RenderTarget& target, sf::RenderStates states)
        const {
