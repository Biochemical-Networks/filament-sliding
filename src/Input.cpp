...
Input::Input(const CommandArgumentHandler &cmd, const std::string fileName)
  ...
  try {
    read();
               ...
               &inputException) // Catch problems caused by an non-existing or
                                // wrongly formatted input file
  {
    try {
      produceDefault();
      read(); // Read again, otherwise data that was already read in before
              // encountering a mistake will remain in m_parameterMap
    } catch (
        InputException &inputException) // Catch problems caused by the user not
                                        // wanting to override the input file,
  ...
  if (cmd.mobileLengthDefined()) {
    m_parameterMap.overrideParameter("lengthMobileMicrotubule",
                                     cmd.getMobileLength());
  }

  if (cmd.numberPassiveDefined()) {
    m_parameterMap.overrideParameter("numberPassiveCrosslinkers",
                                     cmd.getNumberPassive());
  }

  // Write the used input data to an output file, such that the data of the run
  // is stored together with the other information about the run. Do this early
  // (not in the destructor), since a second run can start without the first
  // being finished
...
void Input::read() {
  std::ifstream inputFile(
      m_fileName.c_str()); // Open and close here and don't make it a member
                           // variable, because then the stream would remain
                           // open after failing half way through, and you'll
                           // read from a certain position

  if (!inputFile) {
    throw InputException(std::string(
        "The input file could not be opened")); // Handling the exception can
                                                // include asking for a default
  } else {
    inputFile >> m_parameterMap; // Read the file into memory (parameterMap).
...
void Input::produceDefault() {
                        ...
                        // created
  {
    ...
    if (!inputFile) {
      ...
      inputFile << ParameterMap(); // Create an anonymous object that contains
                                   // the default values for the parameters, and
                                   // print it to the file
    }
  } else {
    throw InputException(
        std::string("User has stopped the creation of a default input file"));
  }
}
...
bool Input::askMakeDefault() {
  ...
  while (redo) {
    ...
    if (numberOfTriesForAnswer > maximumNumberOfTriesForAnswer) {
      throw InputException("User failed to answer question on creating a "
                           "default input file too often.");
    }

    std::cout << "Would you like the program to produce a default input file, "
    ...
    if (std::cin.fail()) // Check if cin was able to extract a character
    {
      ...
      if (answer == 'Y') {
        ...
        std::cout
            << "Please enter yes (Y) or no (n). Be aware of capitalisation.\n";
        redo = true;
      }
    }
  }
...
void Input::setRunName() {
  ...
  try {
    ...
    std::cout << "\"runName\" was not one of the parameters, so the default "
