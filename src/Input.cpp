...
Input::Input(const std::string fileName): m_fileName(fileName) {
    ...
    setRunName(); // Set the name of the current run, first from the input file,
                  // adding labels if the name was previously used
}

Input::~Input() {
    /* Write the used input data to an output file, such that the data of the
     * run is stored together with the other information about the run
     */
...
void Input::read() {
    ...
    if (!inputFile) {
        ...
        inputFile >>
                m_parameterMap; // Read the file into memory (parameterMap). The
...
bool Input::askMakeDefault() {
    ...
    while (redo) {
        std::cout << "Would you like the program to produce a default input "
                     "file, and override the current file? (Y/n)\n";
        char answer = '\0';
        std::cin >> answer;

        std::cin.ignore(
                std::numeric_limits<std::streamsize>::max(),
                '\n'); // Delete all remaining characters in the input stream,
                       // without an assumption on the input size
...
void Input::setRunName() {
    ...
    try {
        ...
        std::cout << "\"runName\" was not one of the parameters, so the "
                     "default run name \"run\" will be used.\n";
        runName = "run";
    }
    int label = 0;
    ...
    while (fileExists(
            m_runName + "." +
            m_fileName)) // If a run with the same name was done before, then
                         // there is a copy of the input file with the name
                         // m_runName+"."+m_fileName
    {
        ++label;
