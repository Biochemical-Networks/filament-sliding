...
SystemState::SystemState(
        ...
        m_maxNumberOfCloseSites(static_cast<int32_t>(
                std::ceil(2 * m_maxStretchPerLatticeSpacing))),
        m_maxStretch(m_maxStretchPerLatticeSpacing * latticeSpacing),
        m_latticeSpacing(latticeSpacing),
        m_springConstant(springConstant),
        m_fixedMicrotubule(
                MicrotubuleType::FIXED,
        ...
        m_nCrosslinkers(
                m_nActiveCrosslinkers + m_nDualCrosslinkers +
                m_nPassiveCrosslinkers),
        m_passiveCrosslinkers(
                m_nPassiveCrosslinkers,
                Crosslinker(Crosslinker::Type::PASSIVE),
        ...
        m_dualCrosslinkers(
                ...
                m_latticeSpacing,
                m_maxStretch),
        m_activeCrosslinkers(
                m_nActiveCrosslinkers,
    ...
    if (externalForceTypeString == "BARRIERFREE") {
        ...
        throw GeneralException(
                "In the SystemState constructor, the given "
...
void SystemState::setMicrotubulePosition(const double initialPosition) {
    ...
    if (m_nCrosslinkers != getNFreeCrosslinkers()) {
        throw GeneralException(
                "SystemState::setMicrotubulePosition() was called "
    ...
    // since no linkers are connected, the only thing the following function
    // calls do is set the borders of the region within the possibilities are
    // valid
...
Crosslinker& SystemState::connectFreeCrosslinker(
    ...
    // connect a free crosslinker in their administration, and to return a
    // pointer to the one that it connected. Then, the pointer can be used to
    // connect the crosslinker in its own administration as well. Done this way,
    // such that the containers don't have to know about the position or
    // microtubule that needs to be connected.
    switch (type) {
    case Crosslinker::Type::PASSIVE:
        p_connectingCrosslinker =
                m_passiveCrosslinkers.connectFromFreeToPartial();
        break;
    ...
    case Crosslinker::Type::ACTIVE:
        p_connectingCrosslinker =
                m_activeCrosslinkers.connectFromFreeToPartial();
        break;
    default:
        throw GeneralException(
    ...
    p_microtubuleToConnect->connectSite(
            locationToConnectTo.position,
            *p_connectingCrosslinker,
            terminusToConnect);
    ...
    m_passiveCrosslinkers.updateConnectionDataFreeToPartial(
            p_connectingCrosslinker);
    m_dualCrosslinkers.updateConnectionDataFreeToPartial(
            p_connectingCrosslinker);
    m_activeCrosslinkers.updateConnectionDataFreeToPartial(
            p_connectingCrosslinker);

    return *p_connectingCrosslinker; // Such that the caller can use this
                                     // specific crosslinker immediately
}
...
void SystemState::disconnectPartiallyConnectedCrosslinker(
    ...
    if (!disconnectingCrosslinker.isPartial()) {
        throw GeneralException(
                "SystemState::disconnectPartiallyConnectedCrosslinker() was "
                "called on "
    ...
    switch (type) {
    case Crosslinker::Type::PASSIVE:
        m_passiveCrosslinkers.disconnectFromPartialToFree(
                disconnectingCrosslinker);
        break;
    case Crosslinker::Type::DUAL:
        m_dualCrosslinkers.disconnectFromPartialToFree(
                disconnectingCrosslinker);
        break;
    case Crosslinker::Type::ACTIVE:
        m_activeCrosslinkers.disconnectFromPartialToFree(
                disconnectingCrosslinker);
        break;
    default:
        throw GeneralException(
...
void SystemState::connectPartiallyConnectedCrosslinker(
    ...
    if (locationThisMicrotubule.microtubule ==
        ...
        throw GeneralException(
                "SystemState::connectPartiallyConnectedCrosslinker() was asked "
                "to "
                "connect a crosslinker twice to one microtubule");
    }

    // Test whether the connection is not outside of the range set by
    // m_maxStretch position is calculated relative to the fixed microtubule,
    // because we want to calculate the total stretch for the check
    ...
    p_microtubuleToConnect->connectSite(
            locationOppositeMicrotubule.position,
            connectingCrosslinker,
            terminusToConnect);
...
void SystemState::disconnectFullyConnectedCrosslinker(
    ...
    switch (type) {
    case Crosslinker::Type::PASSIVE:
        m_passiveCrosslinkers.disconnectFromFullToPartial(
                disconnectingCrosslinker);
        break;
    case Crosslinker::Type::DUAL:
        m_dualCrosslinkers.disconnectFromFullToPartial(
                disconnectingCrosslinker);
        break;
    case Crosslinker::Type::ACTIVE:
        m_activeCrosslinkers.disconnectFromFullToPartial(
                disconnectingCrosslinker);
        break;
    default:
        throw GeneralException(
    ...
    // Disconnect in administration of microtubule
    // Microtubules only keep track whether sites are free or occupied, and
    // which crosslinker is connected to where.
    switch (locationToDisconnectFrom.microtubule) {
    case MicrotubuleType::FIXED:
        m_fixedMicrotubule.disconnectSite(locationToDisconnectFrom.position);
        break;
...
int32_t SystemState::getNFreeCrosslinkersOfType(
...
int32_t SystemState::getNPartialCrosslinkersOfType(
        const Crosslinker::Type type) const {
    switch (type) {
    case Crosslinker::Type::PASSIVE:
...
std::pair<int32_t, int32_t> SystemState::
...
double SystemState::beginningOverlap() const {
    // 0.0 is the position of the minus end of the fixed microtubule (the
    // origin)
...
double SystemState::overlapLength() const {
    ...
    if (overlap <= (-m_maxStretch)) {
        throw GeneralException(
                "The overlap disappeared according to "
                "SystemState::overlapLength()");
    }
    return overlap;
}
...
int32_t SystemState::getFreeSitePosition(
...
#ifdef MYDEBUG
int32_t SystemState::getNSitesToBindPartial(
        const Crosslinker::Type type) const {
...
const std::vector<PossibleFullConnection>& SystemState::getPossibleConnections(
        const Crosslinker::Type type) const {
    switch (type) {
    case Crosslinker::Type::PASSIVE:
    ...
    default:
        throw GeneralException(
                "An incorrect type was passed to "
                "SystemState::getPossibleConnections()");
    }
}

const std::vector<PossiblePartialHop>& SystemState::getPossiblePartialHops(
        const Crosslinker::Type type) const {
    switch (type) {
    case Crosslinker::Type::PASSIVE:
...
const std::vector<PossibleFullHop>& SystemState::getPossibleFullHops(
        const Crosslinker::Type type) const {
    switch (type) {
    case Crosslinker::Type::PASSIVE:
    ...
    default:
        throw GeneralException(
                "An incorrect type was passed to "
                "SystemState::getPossibleFullHops()");
    }
}

const std::vector<FullConnection>& SystemState::getFullConnections(
        const Crosslinker::Type type) const {
    switch (type) {
    case Crosslinker::Type::PASSIVE:
    ...
    default:
        throw GeneralException(
                "An incorrect type was passed to "
                "SystemState::getFullConnections()");
    }
}

const std::vector<Crosslinker*>& SystemState::getPartialLinkers(
        const Crosslinker::Type type) const {
    switch (type) {
    case Crosslinker::Type::PASSIVE:
    ...
    default:
        throw GeneralException(
                "An incorrect type was passed to "
                "SystemState::getPartialLinkers()");
    }
}

const std::vector<Crosslinker*>& SystemState::getFullLinkers(
        const Crosslinker::Type type) const {
    switch (type) {
    case Crosslinker::Type::PASSIVE:
...
void SystemState::updateForceAndEnergy() {
    ...
    m_totalExtensionLinkers =
            totalExtension; // save, for this can be used by the propagator to
                            // integrate the force over the time step (instead
                            // of assuming a constant force during it)
...
double SystemState::externalForceFlatOptimalPath() const {
    ...
    double externalForce = m_springConstant * nFullLinkers *
                           (0.5 * m_latticeSpacing - position);
    // and then the entropic part:
    externalForce += nFullLinkers / m_latticeSpacing *
                     (gsl_sf_psi(
                              nSitesMobileMicrotubule -
...
double SystemState::findExternalForce() const {
                            ...
                            // something
    {
        ...
        case ExternalForceType::SINUS:
            externalForce =
                    -m_externalForceValue *
                    std::sin(
                            2 * m_pi * m_mobileMicrotubule.getPosition() /
                            m_latticeSpacing);
            break;
        ...
        default:
            throw GeneralException(
                    "Caller of SystemState::findExternalForce() asked "
...
void SystemState::TESTunbindAFullCrosslinker(
    ...
    disconnectFullyConnectedCrosslinker(
            *p_disconnectingCrosslinker, terminusToDisconnect);
}
#endif // MYDEBUG
