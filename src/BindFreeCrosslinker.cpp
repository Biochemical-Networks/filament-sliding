...
BindFreeCrosslinker::BindFreeCrosslinker(
        const double rateOneLinkerToOneSite,
...
void BindFreeCrosslinker::setCurrentRate(const SystemState& systemState) {
    // getNFreeSites() gives the number of free sites on both microtubules. Each
    // of those is an (unbiased) option for binding a free linker.
    int32_t nCrosslinkersOfThisType =
            systemState.getNFreeCrosslinkersOfType(m_typeToBind);
    m_currentRate = m_rateOneLinkerToOneSite * systemState.getNFreeSites() *
                    nCrosslinkersOfThisType;
}

SiteLocation BindFreeCrosslinker::whereToConnect(
    ...
    if (freeSiteLabelToConnect >= nFreeSitesFixed) {
        ...
        freeSiteLabelToConnect -= nFreeSitesFixed; // Now it labels a site on
                                                   // the mobile microtubule.
    }

    // Now, we have picked a label to a free site uniformly. Then, map this
    // using a linear (bijective) function to the actual positions on the
    // microtubule where the free sites are
...
void BindFreeCrosslinker::performReaction(
    ...
    systemState.connectFreeCrosslinker(
            m_typeToBind, terminusToConnect, connectLocation);
}
