...
double Statistics::getMean() const {
    ...
    return m_firstValue +
           m_accumulatedDifference / static_cast<double>(m_numberOfSamples);
}
...
double Statistics::getVariance() const {
    ...
    return (m_accumulatedSquaredDifference -
            m_accumulatedDifference * m_accumulatedDifference / n) /
           (n - 1);
}
...
bool Statistics::canReportStatistics() const { return (m_numberOfSamples > 1); }
