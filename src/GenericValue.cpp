        ...
        m_currentType(genericValue.m_currentType) // const member
{
    ...
    *this = genericValue; // Use overloaded operator= for GenericValue to
    ...
    if (&genericValue != this) // Nothing needs to be done at self assignment
    {
        if (!(m_currentType == genericValue.m_currentType)) {
            throw InputException {
                    "A GenericType with non-matching type was tried to be "
                    "assigned"};
        }
...
void GenericValue::copyValue(int32_t& integerValue) const {
    if (m_currentType != AllowedTypes::INTEGER) {
        throw InputException {
                "GenericValue tried to copy a non-integer to an integer "
                "value."};
    }
...
std::string GenericValue::getUnit() const {
    ...
    if (m_currentType == AllowedTypes::NONE) {
        throw InputException(
                "In GenericValue::getUnit(): no value was set yet");
    }
#endif // MYDEBUG
    return m_unit;
}
...
std::ostream& operator<<(std::ostream& out, const GenericValue& genericValue) {
    ...
    case AllowedTypes::NONE:
        throw InputException {
                "A GenericValue with no type was tried to be output."};
        break;
    }
...
std::istream& operator>>(std::istream& in, GenericValue& genericValue) {
     ...
     * Assume the type in the genericValue is correct. If not, then the input
     * will be downcast or fail.
     */
    ...
    if (givenType != genericValue.m_currentType) {
        throw InputException {
                "An input file parameter has a type that does not "
