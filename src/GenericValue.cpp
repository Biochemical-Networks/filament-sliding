...
#include "filament-sliding/InputFileGeometry.hpp" // For the sizes of columns in the input file

// Default copy constructor: copy all members
GenericValue::GenericValue(const GenericValue &genericValue)
    : m_currentType(genericValue.m_currentType) // const member
{
  if (&genericValue == this) {
    throw InputException{"Do not self initialize a GenericValue"};
  }
  *this = genericValue; // Use overloaded operator= for GenericValue to prevent
                        // duplicate code.
}

GenericValue::GenericValue(const std::string &stringValue,
                           const std::string &unit)
    : m_currentType(AllowedTypes::TEXT), m_stringValue(stringValue),
      m_unit(unit) {}

GenericValue::GenericValue(int32_t integerValue, const std::string &unit)
    : m_currentType(AllowedTypes::INTEGER), m_integerValue(integerValue),
      m_unit(unit) {}
  ...
  if (&genericValue != this) // Nothing needs to be done at self assignment
  {
    if (!(m_currentType == genericValue.m_currentType)) {
      throw InputException{
          "A GenericType with non-matching type was tried to be assigned"};
    }
...
// No assignment operators for the allowed types, because a unit always needs to
// be present, and assignment takes one parameter
...
void GenericValue::copyValue(std::string &stringValue) const {
  if (m_currentType != AllowedTypes::TEXT) {
    throw InputException{
        "GenericValue tried to copy a non-string to a string value."};
  }
...
void GenericValue::copyValue(int32_t &integerValue) const {
  if (m_currentType != AllowedTypes::INTEGER) {
    throw InputException{
        "GenericValue tried to copy a non-integer to an integer value."};
  }
...
void GenericValue::copyValue(double &realValue) const {
  if (m_currentType != AllowedTypes::REAL) {
    throw InputException{
        "GenericValue tried to copy a non-real to a real value."};
  }
...
std::ostream &operator<<(std::ostream &out, const GenericValue &genericValue) {
  ...
  case AllowedTypes::NONE:
    throw InputException{
        "A GenericValue with no type was tried to be output."}; // Already
                                                                // covered
                                                                // above, but
                                                                // doesn't hurt
    break;
...
std::istream &operator>>(std::istream &in, GenericValue &genericValue) {
   ...
   * Assume the type in the genericValue is correct. If not, then the input will
   * be downcast or fail.
   */
  ...
  default:
    throw InputException{
        "Input was tried to be put into a GenericValue with no type."};
    break;
  }
   ...
   * If no string is recognized or the type does not correspond to what was
  ...
  // The givenType should correspond to the type of the GenericValue that this
  // variable will be stored in
  if (givenType != genericValue.m_currentType) {
    throw InputException{"An input file parameter has a type that does not "
                         "correspond to the GenericValue it is stored in"};
  }
...
AllowedTypes convertToType(const std::string &type) {
  ...
  if (type == "string" || type == "STRING" || type == "String" ||
    ...
    throw InputException{
        "An input file parameter has a type that is not recognized."};
  }
