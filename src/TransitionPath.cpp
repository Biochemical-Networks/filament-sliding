...
#include "filament-sliding/GeneralException/GeneralException.hpp"
...
void TransitionPath::addPoint(const double time, const double mobilePosition,
...
double TransitionPath::getMobilePosition(const int32_t label) const {
  try {
    ...
    throw GeneralException(
        std::string(
            "TransitionPath::getMobilePosition() label went out of range: ") +
...
int32_t TransitionPath::getNRightPullingLinkers(const int32_t label) const {
  try {
    ...
    throw GeneralException(
        std::string("TransitionPath::getNRightPullingLinkers() label went out "
                    "of range: ") +
        std::string(outOfRange.what()));
  }
}
