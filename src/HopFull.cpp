...
HopFull::HopFull(const double baseRateHead, const double baseRateTail,
    ...
    : Reaction(), // Use the base rates instead of the elementaryRate
      m_typeToHop(typeToHop), m_springConstant(springConstant),
      m_headHopToPlusBaseRate(baseRateHead *
                              std::exp(headHopToPlusBiasEnergy * 0.5)),
      m_headHopToMinusBaseRate(baseRateHead *
                               std::exp(-headHopToPlusBiasEnergy * 0.5)),
      m_tailHopToPlusBaseRate(baseRateTail *
                              std::exp(tailHopToPlusBiasEnergy * 0.5)),
      m_tailHopToMinusBaseRate(baseRateTail *
                               std::exp(-tailHopToPlusBiasEnergy * 0.5)),
...
void HopFull::setCurrentRate(const SystemState &systemState) {
  const std::vector<PossibleFullHop> &possibleFullHops =
      systemState.getPossibleFullHops(m_typeToHop);
  ...
  for (const PossibleFullHop &possibleFullHop : possibleFullHops) {
    ...
    // First, there can be a non-equilibrium driving in one direction, causing
    // causing the rate to be biased. Second, the rate depends on the energy of
    // stretching, 0.5*springConstant*extension^2 The influence of this energy
    // is spread evenly over the forward and backward reactions, explaining the
    // extra factor 0.5 in the exponent.
    double rate = getBaseRateToHop(possibleFullHop.terminusToHop,
                                   possibleFullHop.direction,
                                   possibleFullHop.awayFromNeighbour);
    rate *=
        std::exp(0.25 * m_springConstant *
...
double HopFull::getBaseRateToHop(const Crosslinker::Terminus terminusToHop,
  ...
  if (terminusToHop == Crosslinker::Terminus::HEAD &&
    ...
    throw GeneralException(
        "HopFull::getBaseRateToHop() was called with improper parameters.");
  }
}

const PossibleFullHop &HopFull::whichHop(const SystemState &systemState,
  ...
  if (possibleFullHops.size() != m_individualRates.size()) {
    throw GeneralException(
        "HopFull::whichHop() was called with an outdated vector");
  }
#endif // MYDEBUG
...
void HopFull::performReaction(SystemState &systemState,
                              RandomGenerator &generator) {
  PossibleFullHop hopToMake = whichHop(systemState, generator);
  // A hop consists of disconnecting the crosslinker at one terminus, and then
  // connecting that terminus again
  systemState.disconnectFullyConnectedCrosslinker(*hopToMake.p_fullLinker,
                                                  hopToMake.terminusToHop);
  systemState.connectPartiallyConnectedCrosslinker(*hopToMake.p_fullLinker,
