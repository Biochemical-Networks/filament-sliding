...
void Extremity::connect(const SiteLocation siteToConnectTo) {
    ...
    if (m_connected) {
        throw GeneralException(
                ...
                "connected again in Extremity::connect()");
    }
#endif // MYDEBUG
    m_connected = true;
...
void Extremity::changePosition(const SiteLocation siteToConnectTo) {
    ...
    if (!m_connected) {
        ...
        throw GeneralException(
                "A connected extremity was moved to the other microtubule in "
                "Extremity::changePosition()");
    }
#endif // MYDEBUG
    m_sitePosition = siteToConnectTo.position;
...
void Extremity::disconnect() {
    ...
    if (!m_connected) {
        throw GeneralException(
                "An unconnected extremity was attempted to be disconnected in "
                "Extremity::disconnect()");
    }
#endif // MYDEBUG
    m_connected = false;
}
...
MicrotubuleType Extremity::getMicrotubuleConnectedTo() const {
    ...
    if (!m_connected) {
        throw GeneralException(
                "Extremity::getMicrotubuleConnectedTo() was called on a "
                "disconnected extremity");
    }
#endif // MYDEBUG
