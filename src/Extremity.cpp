...
void Extremity::connect(const SiteLocation siteToConnectTo) {
  ...
  if (m_connected) {
    throw GeneralException(
        "An extremity that was already connected is attempted to be connected "
...
#ifdef MYDEBUG
  if (!m_connected) {
    throw GeneralException("An unconnected extremity was tried to be moved in "
                           "Extremity::changePosition()");
  } else if (siteToConnectTo.microtubule != m_connectedTo) {
    throw GeneralException("A connected extremity was moved to the other "
...
#ifdef MYDEBUG
  if (!m_connected) {
    throw GeneralException("An unconnected extremity was attempted to be "
...
#ifdef MYDEBUG
  if (!m_connected) {
    throw GeneralException("Extremity::getMicrotubuleConnectedTo() was called "
