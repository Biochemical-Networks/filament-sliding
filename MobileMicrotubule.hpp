...
private:
    ...
    // An integer giving the position of the free energy valley where the mobile microtubule is currently at, in units of the lattice spacing
    // Typically, the microtubule will fluctuate around a point where all springs (crosslinkers) can be relaxed.
    // This is where the two microtubules exactly lie on top of each other.
    // Say that the microtubule has crossed the free energy barrier when the position is exactly one delta further than the old m_currentAttractorPosition.
    // In that case, the microtubule has passed the barrier and even half of the valley:
